...
def get_distribution_metrics(source_file_path: str,
    ...
    XY_bins_img : np.ndarray
        3D array of XY bins for the first organelle (same for all organelles since the same bins are applied to each)
    XY_wedges_img : np.ndarray
        3D array of XY wedges for the first organelle (same for all organelles since the same wedges are applied to each)
    ...
    return final_dist_tab, XY_bins_imgs[0], XY_wedges_imgs[0]
...
def batch_process_distribution_quant(dataset_name: str,
    ...
    dist_path = quant_path / f"{dataset_name}-organelle_distribution_metrics.csv"
...
def batch_distribution_summary_stats(out_prefix: str,
    ...
    for loc in csv_path_list:
        ...
        # find the unique datasets in this location based on the prefixes before "-organelle_distribution_metrics"
        prefixes = set(f.name.split("-organelle_distribution_metrics")[0] for f in files_store if "-organelle_distribution_metrics" in f.name)
        print(f"Found the following datasets in {loc}:", prefixes)
        for prefix in prefixes:
            ds_count += 1
            # select only the files from this dataset
            files_subset = [f for f in files_store if f.name.startswith(prefix +"-organelle_distribution_metrics")]
    ...
    if (Path(out_path) / f"{out_prefix}-per_org_distribution_summarystats.csv").exists():
        raise FileExistsError(f"CAUTION: {out_prefix}-per_org_distribution_summarystats.csv already exists and will not be overwritten. Move the existing file, change the `out_prefix` or `out_path` to continue without error.")
    else:
        dist_summary.to_csv(str(out_path) + f"/{out_prefix}-per_org_distribution_summarystats.csv")
    ...
    if (Path(out_path) / f"{out_prefix}-distribution_summarystats.csv").exists():
        ...
        dist_final.to_csv(str(out_path) + f"/{out_prefix}-distribution_summarystats.csv", mode='x')
        print(f"Exported distribution summary statistics (after unstacking) to {out_path}/{out_prefix}-distribution_summarystats.csv")
